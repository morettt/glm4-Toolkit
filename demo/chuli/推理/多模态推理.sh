...
MODEL_PATH="/root/autodl-tmp/glm-4v-9b"
...
if [ -d "$MODEL_PATH" ]; then
    echo "发现已有glm-4v-9b模型文件，直接运行推理程序。"
    python /root/GLM-4/basic_demo/trans_cli_vision_demo.py
    echo "程序已退出。"
    exit 0
fi
...
# 下载模型文件并运行推理程序
echo "你好！发现您并没有glm-4v-9b模型文件，需要我为您下载吗？输入：yes 或者 no"
read user_input
if [ "$user_input" = "yes" ]; then
    cd "$TARGET_DIR"
    if cg down xxxiu/glm-4v-9b; then
        python /root/GLM-4/basic_demo/trans_cli_vision_demo.py
    else
        echo "下载模型失败。"
        exit 1
